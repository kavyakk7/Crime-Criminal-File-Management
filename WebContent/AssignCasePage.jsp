...
<div class="panel panel-default ">
  ...
  <center><h1>Hello <i><%= c.getPs_name() %></i> Sir,  Welcome!</h1>
		...
		<form action="AssignCaseToPolicePage.jsp" method="POST">
  	<select name="case">
	...
	String QueryString = "Select case_assigned from police where ps_name='"+c.getPs_name()+"'";
	rs = statement.executeQuery(QueryString);
	while(rs.next())
	{
		cases = rs.getString(1);
	}
		//String cases = c.getPs_case_assigned();
  		String[] caseid = cases.split(",");
  		for(int i=0;i<caseid.length;i++)
  		{
  			%>
  			<option><%=caseid[i] %></option>
  			<%
  		}
  ...
  <div class="container" style="padding-left:60px;">
		...
		<input type="submit" name="sbmt" class="btn btn-primary col-sm-4" value="ASSIGN CASE"><br>
	</form><br>
  	</div>
  </div>
  <br><br>
